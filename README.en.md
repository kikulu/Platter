...
| 📚 Knowledge base (separate window)    | Ships with 56 built-in prompt templates across 8 domains (enterprise ops, medical software R&D, academic writing, research proposals, project development, spec-driven development (SDD), OpenSpec, enterprise role templates), full-text search, filterable by tags, exportable as Markdown or JSON                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 📦 Staged prompt bundles               | Bundles can split their steps into stages (e.g. "Plan → Write → Submit"); "Copy this stage" copies every prompt in a stage to paste into one AI conversation. Ships with 7 staged bundle templates (academic writing, research proposal, software project, medical-device software compliance, recurring project communication, spec-driven development (SDD), OpenSpec change workflow)                                                                                                                                                                                                                                                                                                                                                            |
...
4. Save prompts you use often into the "Knowledge Base" so you can copy
   ...
   research proposal, running a project...) use a "Bundle": 7 staged
   bundles are built in (including Spec-Driven Development: principles →
   spec → plan → tasks → implement, and the OpenSpec change workflow), and in the bundle editor you can add stage
...
│   ├── default-project-templates.json # 5 built-in software-development project templates (SDD / OpenSpec / Scrum / Waterfall / MVP)
│   ├── default-knowledge-base.json # 56 built-in prompt templates (40 general + 16 role-based) + 7 staged bundle templates
