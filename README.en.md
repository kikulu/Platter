...
| 📚 Knowledge base (separate window)    | Ships with 48 built-in prompt templates across 7 domains (enterprise ops, medical software R&D, academic writing, research proposals, project development, spec-driven development (SDD), enterprise role templates), full-text search, filterable by tags, exportable as Markdown or JSON                                                                                                                                                                                  |
| 📦 Staged prompt bundles               | Bundles can split their steps into stages (e.g. "Plan → Write → Submit"); "Copy this stage" copies every prompt in a stage to paste into one AI conversation. Ships with 6 staged bundle templates (academic writing, research proposal, software project, medical-device software compliance, recurring project communication, spec-driven development (SDD))                                                                                                              |
...
| 📋 Project templates & staged workflow | "Project Plans" can create a project from a template: 4 built-in software-development templates (Spec-Driven Development, Agile Scrum, Waterfall, MVP prototype). Enter a project topic, then work through each stage's prompts in the "Workflow" tab — copy (topic and earlier outputs filled in automatically) → paste the AI's output back → mark done → next step. Every step is mirrored as a project task, and you can save an adjusted workflow as your own template |
...
4. Save prompts you use often into the "Knowledge Base" so you can copy
   ...
   prompt into the chat box). For multi-step work (writing a paper, a
   research proposal, running a project...) use a "Bundle": 6 staged
   bundles are built in (including Spec-Driven Development: principles →
   spec → plan → tasks → implement), and in the bundle editor you can add stage
   headings with "+ Stage" and move steps between stages with ↑↓. Click
   "Copy This Stage" to copy every prompt in a stage at once, and use
   the checkbox in front of each step to track progress.
...
│   ├── default-project-templates.json # 4 built-in software-development project templates (SDD / Scrum / Waterfall / MVP)
│   ├── default-knowledge-base.json # 48 built-in prompt templates (32 general + 16 role-based) + 6 staged bundle templates
