...
| 📚 知識庫（獨立視窗）   | 內建 48 組跨領域提示詞範本（企業日常作業/醫療軟體研發/論文寫作/研究計畫/專案開發/SDD 規格驅動開發/企業角色範本），支援全文搜尋、標籤篩選，可匯出成 Markdown 或 JSON                                                                                                                                                  |
| 📦 分階段提示詞套餐     | 套餐可以把步驟分成多個階段（例如「規劃 → 撰寫 → 投稿」），每個階段可「複製本階段」一次貼進同一段 AI 對話；內建 6 組分階段套餐範本（論文寫作、研究計畫、軟體專案開發、醫療器材軟體合規、專案例行溝通、SDD 規格驅動開發）                                                                                              |
| 🧑‍💼 企業角色提示詞範本   | 內建 8 種常見企業職務角色（HR／行銷／業務／客服／PM／工程師／財務／高階主管），每種角色各配好 2 組任務範本，每組都拆成「系統提示詞」與「使用者提示詞」，開機即用，側邊欄「預設提示詞」可分別一鍵複製                                                                                                                 |
| 📋 專案範本與階段流程   | 「專案計畫管理」可從範本建立專案：內建 SDD 規格驅動開發、敏捷 Scrum、傳統瀑布式、MVP 快速原型 4 組軟體開發範本。輸入專案主題後，在「階段流程」分頁依各階段的提示詞逐步執行——複製（自動帶入主題與前面步驟的產出）→ 貼回 AI 的產出 → 標記完成 → 下一步，每個步驟同步成一個專案任務；也能把調整過的流程另存為自己的範本 |
...
4. 常用的 prompt 存進「知識庫」，之後點開複製貼上就好，不用重打。內建的
   ...
   「套餐」：內建 6 組分階段套餐（含 SDD 規格驅動開發：原則 → 規格 → 規劃 → 任務 → 實作），也能自己在套餐編輯器用「＋ 階段」加
...
│   ├── default-project-templates.json # 內建4組軟體開發專案範本（SDD／Scrum／瀑布式／MVP）
│   ├── default-knowledge-base.json # 內建48組提示詞範本（一般32組＋企業角色範本16組）＋6組分階段套餐範本
