...
| 📦 分階段提示詞套餐     | 套餐可以把步驟分成多個階段（例如「規劃 → 撰寫 → 投稿」），每個階段可「複製本階段」一次貼進同一段 AI 對話；內建 5 組分階段套餐範本（論文寫作、研究計畫、軟體專案開發、醫療器材軟體合規、專案例行溝通） |
...
4. 常用的 prompt 存進「知識庫」，之後點開複製貼上就好，不用重打。內建的
   ...
   「套餐」：內建 5 組分階段套餐，也能自己在套餐編輯器用「＋ 階段」加
...
│   ├── default-knowledge-base.json # 內建41組提示詞範本（一般25組＋企業角色範本16組）＋5組分階段套餐範本
