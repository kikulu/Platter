...
<p align="center">
  ...
  也能接上 Ollama、LM Studio、Stable Diffusion WebUI、ComfyUI 等本地端
  AI 服務，各自獨立、互不干擾。
</p>
...
| 🖥️ 本地端 AI 服務       | 側邊欄獨立群組，可新增自訂名稱＋網址的本地服務（Ollama／LM Studio／Stable Diffusion WebUI／ComfyUI 等），標記 LLM／文生圖／自訂分類；跟一般帳號一樣用內嵌視窗開啟；「設定」裡另有集中管理區塊可測連線、編輯、刪除                                                                                                                                                                                                                                                                                              |
| 🔐 登入狀態持久化       | 重開程式不用重新登入                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
1. 開啟 App，點側邊欄「＋ 新增帳號」，選平台（Claude/ChatGPT/Gemini/Grok）、
   取個自訂名稱，跳出的網頁裡正常登入即可。若要接本地端 AI 服務
   （Ollama／LM Studio／Stable Diffusion WebUI／ComfyUI），改用側邊欄
   另一個獨立群組「本地端 AI 服務」的「＋ 新增本地服務」，多填服務類型
   跟網址（例如 `http://localhost:11434`）即可，網址事後可在「設定」
   裡改。
...
- **本地端 AI 服務**（`platform === 'local'`）：Claude/ChatGPT/Gemini/
  Grok 載入固定網址（`lib/constants.js` 的 `PLATFORM_URLS`），本地服務
  則載入使用者自訂的網址（帳號自己的 `url` 欄位），改網址不用整個帳號
  刪掉重建（session partition、登入狀態都保留）。連線測試走 Electron
  `net.request`（不經頁面 CORS），只看連得上、不管狀態碼。
