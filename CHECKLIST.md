...
> **這份文件已棄用，請勿依照它來追蹤進度或重現專案。**
>
> 這是專案早期的草稿式檢核表，跟目前實際的 `BUILD_PLAN.md`（8 個階段，
> 每階段附有可操作的檢核表且已全部驗收打勾）階段編號與內容並不一致
> （例如下面的「階段6：多國語系」在目前的 `BUILD_PLAN.md` 裡是併在
> Stage 4 一起做；下面提到的 `lib/utils.js`、`test/utils.test.js`、
> `.eslintrc.json` 等項目，是後來才依照實際需求真正建置，跟這份草稿
> 描述的內容也不完全相同）。所有檢核框從一開始就沒有被更新過。
>
> - 想知道目前建置到哪個階段、驗收過哪些項目 → 看 `BUILD_PLAN.md`。
> - 想知道目前完整規格長怎樣 → 看 `PROJECT_SPEC.md`。
> - 想繼續開發新功能／修正既有功能 → 看
>   `NEW_FEATURE_BUILD_PROMPT.md` / `FIX_EXISTING_FEATURE_PROMPT.md`。
>
> 保留這份檔案只是為了不遺失早期規劃的歷史紀錄，之後也可以考慮直接刪除。
