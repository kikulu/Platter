 ...
 *       artifact,                 // 選填：這一步的產出要匯出成 OpenSpec 變更資料夾裡的哪個檔案（見 lib/openspec.js）
 *       useBaseContext            // 選填：這一步的提示詞要不要帶入「既有規格」（workflow.baseContext）
 *     }],
 *     baseContext                 // 選填：從專案匯入的 openspec/specs 現有規格 { source, text, capabilityCount, ... }
 *   }
 */
...
function resolveTemplateStep(step, resolveItem) {
  ...
  // 選填的整合旗標：artifact = 產出要匯出成 OpenSpec 變更資料夾裡的哪個檔案；
  // useBaseContext = 提示詞要帶入專案匯入的既有規格
  const artifact =
    typeof step.artifact === 'string' && step.artifact.trim()
      ? step.artifact.trim()
      : null;
  const useBaseContext = step.useBaseContext === true;
  ...
  return { title: title || '(未命名步驟)', prompt, artifact, useBaseContext };
}
  ...
  (Array.isArray(template.stages) ? template.stages : []).forEach((stage) => {
    ...
    (Array.isArray(stage.steps) ? stage.steps : []).forEach((rawStep) => {
      ...
      steps.push({
        ...
        useBaseContext: resolved.useBaseContext,
      });
    });
  });
...
function composeStepPrompt(workflow, stepId) {
  ...
  // 既有規格（專案匯入的 openspec/specs）：只有範本標了 useBaseContext 的步驟才帶入
  const baseText = workflow.baseContext && String(workflow.baseContext.text || '').trim();
  if (step.useBaseContext && baseText) {
    parts.push(`# 既有規格（專案 openspec/specs 目前的內容）\n${baseText}`);
  }
...
/**
 * 任務清單那一側改了任務狀態（使用者直接在任務清單切換、或按「儲存專案」）時，把狀態反向
 * 同步給對應的 workflow 步驟，讓「步驟狀態」與「任務狀態」永遠一致（另一個方向見
 * lib/ipc/projects.js 的 projects:workflow:setStepStatus）。回傳有沒有任何步驟被改動。
 * 對應任務已被刪掉的步驟維持原狀。
 */
function syncStepStatusFromTasks(workflow, tasks) {
  let changed = false;
  const byId = new Map((tasks || []).map((t) => [t.id, t]));
  workflow.steps.forEach((step) => {
    const task = step.taskId && byId.get(step.taskId);
    if (task && STEP_STATUSES.includes(task.status) && step.status !== task.status) {
...
function templateFromWorkflow(workflow, { name, description } = {}) {
  const stages = groupStepsByStage(workflow.steps).map((run) => ({
    ...
    steps: run.steps.map((s) => {
      const step = { title: s.title, prompt: s.prompt };
      // 保留 OpenSpec 整合旗標，用另存的範本建立的專案才能繼續匯出／帶入既有規格
      if (s.artifact) step.artifact = s.artifact;
      if (s.useBaseContext) step.useBaseContext = true;
      return step;
    }),
  }));
  return {
...
function normalizeWorkflow(workflow) {
  ...
  workflow.steps.forEach((s) => {
    ...
    s.artifact = typeof s.artifact === 'string' && s.artifact ? s.artifact : null;
    s.useBaseContext = s.useBaseContext === true;
  });
  if (workflow.baseContext) {
    const b = workflow.baseContext;
    if (typeof b === 'object' && typeof b.text === 'string' && b.text.trim()) {
      b.source = typeof b.source === 'string' ? b.source : '';
      b.capabilityCount = Number(b.capabilityCount) || 0;
