...
const state = {
  ...
  appState: {
    // { id, platform, name, roleId }
    // platform === 'local'（本地端 AI 服務，如 Ollama／SD WebUI／ComfyUI）
    // 多兩個欄位： { url, serviceType: 'llm' | 'image' | 'custom' }
    accounts: [],
    ui: { ...DEFAULT_UI_STATE },
  ...
  // --- 跨模組快速搜尋（命令面板）「跳到項目」用的暫存，見 lib/ipc/search.js ---
  // { knowledge?: {id, tab}, documents?: {id}, conversations?: {id}, projects?: {id, tab} }
  pendingSelections: {},
