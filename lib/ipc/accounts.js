...
const { addAccount, removeAccount, switchAccount, layoutActiveView } = require('../windows');
...
function registerAccountsIpc(ipcMain) {
  ipcMain.handle('accounts:list', () => {
    return state.appState.accounts.map((a) => ({ ...a, active: a.id === state.activeAccountId }));
  });
  ...
  ipcMain.handle('roles:save', (e, role) => {
    ...
    if (role.id) {
      ...
      if (idx >= 0) {
        state.appState.roles[idx] = { ...state.appState.roles[idx], ...role, updatedAt: now };
      } else {
