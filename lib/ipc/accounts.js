...
function registerAccountsIpc(ipcMain) {
  ...
  ipcMain.handle('accounts:add', (e, { platform, name, roleId, extra }) => {
    const account = addAccount(platform, name, roleId, extra);
    logAudit('account', 'add', `新增帳號「${account.name}」(${account.platform})`);
    broadcastToAllWindows('accounts:changed');
    return account;
  });

  // --- 本地端 AI 服務（platform === 'local'）專用：編輯網址/名稱/類型、
  // 測試連線。帳號本身的新增/切換/移除沿用上面既有的 accounts:* API。
  ipcMain.handle('localServices:update', (e, { id, patch }) => {
    const account = updateLocalService(id, patch || {});
    if (account) {
      logAudit('account', 'update', `更新本地服務「${account.name}」`);
      broadcastToAllWindows('accounts:changed');
    }
    return account;
  });

  ipcMain.handle('localServices:test', (e, url) => testLocalServiceConnection(url));
