  ...
  ipcMain.handle('knowledge:exportAll', async (e, format) => {
    ...
    if (format === 'json') {
      ...
      const groupBlocks = kb.groups.map((g) => {
        ...
        // 步驟編號跨階段連續；有分階段的套餐在每個階段前加一行「### 階段名稱」，
        // 沒有分階段（只有一個 stage 為空字串的區段）就維持原本的純條列格式。
        let stepNo = 0;
        const steps = groupStepsByStage(g.steps)
          .map((run) => {
            const lines = run.steps.map((s) => {
              const item = itemsById.get(s.itemId);
              stepNo += 1;
              return `${stepNo}. [${s.checked ? 'x' : ' '}] ${item ? item.title : '(已刪除的項目)'}`;
            });
            return run.stage
              ? [`### ${run.stage}`, '', ...lines].join('\n')
              : lines.join('\n');
          })
          .join('\n\n');
