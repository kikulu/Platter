...
const { registerAccountMeta } = require('../windows');
const { pickSelectorSample, testCaptureSelector } = require('../conversationCapture');
...
function registerSettingsIpc(ipcMain) {
  ...
  ipcMain.handle('settings:addExtension', async () => {
    ...
    state.accountViews.forEach((entry) => {
      if (!entry.view) return; // 還沒載入頁面（懶載入），沒有 session 可以套用，等它載入時會自己套用目前的擴充功能清單
      const ses = entry.view.webContents.session;
      ses.loadExtension(ext.path, { allowFileAccess: true }).catch((err) => {
        logError(
          'extensions:load',
          `安裝擴充功能後套用到帳號「${entry.name}」失敗: ${ext.path}`,
          err
        );
      });
    });
  ...
  ipcMain.handle('settings:toggleExtension', (e, { id, enabled }) => {
    ...
    saveAppState();
    state.accountViews.forEach((entry) => {
      if (!entry.view) return; // 還沒載入頁面（懶載入），等它載入時會自己套用目前的擴充功能清單
      const ses = entry.view.webContents.session;
      if (enabled) {
        ses.loadExtension(ext.path, { allowFileAccess: true }).catch((err) => {
          logError(
            'extensions:load',
            `啟用擴充功能「${ext.name}」失敗: ${ext.path}`,
            err
          );
        });
      } else {
  ...
  ipcMain.handle('settings:removeExtension', (e, id) => {
    ...
    if (ext) {
      state.accountViews.forEach((entry) => {
        if (!entry.view) return; // 還沒載入頁面（懶載入），沒有 session 可以移除
        const ses = entry.view.webContents.session;
        const loaded = ses.getAllExtensions().find((x) => x.path === ext.path);
        if (loaded) ses.removeExtension(loaded.id);
      });
  ...
  // 「匯出對話時同時加入知識庫」勾選框的持久化偏好，見
  // lib/conversationCapture.js 的 exportCurrentConversation()。
  ipcMain.handle('settings:setAutoAddToKnowledgeOnExport', (e, enabled) => {
    state.appState.ui.autoAddToKnowledgeOnExport = !!enabled;
    saveAppState();
    return true;
  });
  ...
  ipcMain.handle('settings:exportBackup', async () => {
    const backup = {
      ...
      projectTemplates: loadProjectTemplates(), // 使用者自訂的專案範本（內建範本不用備份）
  ...
  ipcMain.handle('settings:importBackup', async () => {
    ...
    // 使用者自訂專案範本：已存在的 id 略過（舊備份沒有這個欄位就跳過）
    const projectTemplates = loadProjectTemplates();
    const existingTemplateIds = new Set(projectTemplates.map((t) => t.id));
    (backup.projectTemplates || []).forEach((t) => {
      if (t && t.id && !existingTemplateIds.has(t.id)) projectTemplates.push(t);
    });
  ...
  // 測試擷取預覽：用「還沒儲存」的 selector 直接對目前開著的同平台帳號跑一次
  // 擷取，回傳完整結果（含訊息列表跟 debug 診斷數字）給 UI 即時顯示，
  // 不用真的匯出、也不影響 selectors.json / 文件庫 / 日誌。
  ipcMain.handle('settings:testCaptureSelector', (e, { platform, selector }) =>
  ...
  ipcMain.handle('settings:clearCache', async () => {
    ...
    let loadedCount = 0;
    state.accountViews.forEach((entry) => {
      if (!entry.view) return; // 還沒載入頁面（懶載入），沒有 session 需要清快取
    ...
    await Promise.all(
      Array.from(sessions).map((ses) =>
        ses
          .clearCache()
          .catch((err) => logError('settings:clearCache', '清除快取失敗', err))
      )
    );
    logAudit(
      'settings',
      'clearCache',
      `清除快取（含目前已載入的 ${loadedCount} 個帳號）`
    );
    return true;
  });
}
