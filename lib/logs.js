...
async function initLogsDatabase() {
  ...
  state.pendingLogEntries
    .splice(0)
    .forEach(({ kind, entry }) => appendLogEntry(kind, entry));
}
...
function migrateLegacyJsonLogsIfNeeded() {
  ...
  const hasExistingRows =
    (errorCountRows[0] && errorCountRows[0].c > 0) ||
    (auditCountRows[0] && auditCountRows[0].c > 0);
  ...
  try {
    ...
    console.log(
      '[logs] 已把舊版 logs.json 搬進 SQLite，原始檔案改名為 logs.json.migrated 留底'
    );
  } catch (err) {
...
function loadLogs() {
  ...
  return {
    errors: queryAll(
      state.logsDb,
      'SELECT id, timestamp, scope, message, stack FROM errors ORDER BY timestamp ASC'
    ),
    audits: queryAll(
      state.logsDb,
      'SELECT id, timestamp, category, action, detail FROM audits ORDER BY timestamp ASC'
    ),
  };
}
...
function appendLogEntry(kind, entry) {
  ...
  if (kind === 'errors') {
    state.logsDb.run(
      ...
      [id, timestamp, entry.scope || null, entry.message || null, entry.stack || null]
    );
    trimLogTable('errors');
  } else if (kind === 'audits') {
    state.logsDb.run(
      'INSERT INTO audits (id, timestamp, category, action, detail) VALUES (?, ?, ?, ?, ?)',
      [id, timestamp, entry.category || null, entry.action || null, entry.detail || null]
    );
    trimLogTable('audits');
  }
  saveLogsDatabase();
