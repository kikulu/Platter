...
function applyIncrementalDefaultSeeds(knowledgeData) {
  ...
  const seededRecord = readJSONSafe(seededDefaultsPath(), null);
  let seededIds = Array.isArray(seededRecord && seededRecord.knowledgeBase)
  ...
  const seededGroupSet = new Set(
    Array.isArray(seededRecord && seededRecord.knowledgeGroups)
      ? seededRecord.knowledgeGroups
      : []
  );
  ...
  writeJSONSafe(seededDefaultsPath(), {
